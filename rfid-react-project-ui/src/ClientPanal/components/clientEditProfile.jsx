...
import { updateClient } from "../service/api"; // Assuming userUpdate API is imported from api.js
...
const ClientEditProfile = () => {
  ...
  useEffect(() => {
    ...
    if (storedUserData) {
      try {
        const parsedClientData = JSON.parse(storedUserData);
        setClientData(parsedClientData);  // Set user data in state
        console.log(parsedClientData);
        // Initialize form state with data from cookie
        setName(parsedClientData.client_name); // Initialize name state
        setEmail(parsedClientData.client_email); // Initialize email state
        setMobile(parsedClientData.client_mobile); // Initialize mobile state
      } catch (error) {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const data = await updateClient(clientData.client_id, name, email, mobile);
